...
export const LegalFooter = forwardRef<HTMLDivElement, LegalFooterProps>(({ className, children }, ref) => {
  ...
  console.log("divContainerStyles", divContainerStyles);

  return (
    <div
      ref={divContainerRef}
      className={cn("flex w-full items-center justify-between px-4 pb-4 pt-8 text-[10px] lg:px-0", className, makeMultiLine && "flex-col")}
    >
      <span className={cn("text-gray-600 pr-1", makeMultiLine && "pr-0 pb-1")}>
        {process.env.NEXT_PUBLIC_APP_TITLE
          ...
          : ""}
        © {new Date().getFullYear()} {process.env.NEXT_PUBLIC_APP_TITLE || "Your Company"} Inc. All rights reserved.
      </span>
