...
export async function sendRenewEmailTokenUrlEmail(
  email: string,
  ...
  const url = urlProvided ?? `${process.env.NEXT_PUBLIC_ROOT_DOMAIN_URL_FULL}/auth/verify-email?signUp=1&email=` + email + "&code=" + verificationToken;
  const body =
    bodyProvided ??
    `
        <div style="font-family: Arial, sans-serif; padding: 20px;">
          ...
          <h2 style="color: #333333; margin-bottom: 20px;">You're almost done!</h2>
          <p style="color: #333333;">To complete your registration, please click the button below:</p>
          <a href="${url}" style="display: inline-block; padding: 10px 20px; background-color: #1da1f2; color: #ffffff; text-decoration: none; margin-top: 20px;">Continue</a>
